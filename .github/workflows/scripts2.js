...
// URL fija del archivo en GitHub
const URL_GITHUB = "https://github.com/usuario/repo/raw/main/productos.xlsx";

document.addEventListener('DOMContentLoaded', function() {
    // Cargar el archivo automáticamente desde la URL fija al cargar la página
    cargarDesdeGitHub(URL_GITHUB);
});
...
function cargarDesdeGitHub(url) {
    ...
    fetch(url)
        ...
        .catch(error => {
            console.error('Error al cargar el archivo desde GitHub:', error);
            alert('Hubo un error al cargar el archivo desde GitHub.');
        });
}
...
function generarEtiquetas() {
    ...
    etiquetasPreAgregadas.forEach((etiqueta) => {
        ...
        div.innerHTML = `
            <p style="text-align:center; font-size: 18px; font-weight: bold;">${etiqueta.codigo}</p>
            <svg class="barcode" style="display: block; margin: 0 auto;"></svg>
            <p style="text-align:center;">${etiqueta.descripcion}</p>
            <p style="text-align:center;">Unidades: ${etiqueta.unidades}</p>
        `;
